...
const bookingCode    = document.querySelector("#store-code"),
   ...
   instalCheck       = document.querySelector('#Instalation'),
...
window.addEventListener('DOMContentLoaded', function () {
   ...
   //asTaskAdd.onclick    = function() {taskSetAs()}
   ...
   requestDate.onchange = function () {
      ...
      const dates             = new Date(this.value).setDate(getDate + 1)
      instalDate.valueAsDate  = new Date(dates)
      instalDate.min    = new Date(new Date().setDate(new Date(this.value).getDate() - 7)).toISOString().split("T")[0]
   }
...
function checkAs () {
   ...
   if (asPhone.value.length < 10 || asPhone.value.length > 13 || !asPhone.startsWith('08')) status = false
   return status
}
...
function asSeeDetail (IDX) {
   console.log(IDX)
   const data  = AS.findById(IDX),
